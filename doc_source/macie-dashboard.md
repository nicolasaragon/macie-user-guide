...
For more information about PII\-based object classification, see [Classify Your Data with Macie](macie-classify-data.md)\.
...
1. For example, let's generate a list of S3 objects with low PII priority\. In the **S3 objects by PII priority** list, choose the looking glass icon next to the low PII priority\.
   ...
   The results of this query are also displayed\. You can then modify the query result controls available on the **Research** tab and run the query again, and conduct in\-depth investigative research of the generated results\. For more information, see [Using the Macie Research Tab](macie-research.md)\.
...
1. For example, let's generate a list of user sessions in which **Client\.InvalidPermission\.NotFound** error is present\. Double\-click on the square next to **Client\.InvalidPermission\.NotFound**\. 
   ...
   `eventNameErrorCode.secondary:"Client.InvalidPermission.NotFound" x AND x @timestamp:[now-60d TO now] x`
...
You can then modify the query result controls available on the **Research** tab and run the query again, and conduct in\-depth investigative research of the generated results\. For more information, see [Using the Macie Research Tab](macie-research.md)\. 
...
`eventNameIsp.key.keyword:"AssumeRole" x`
